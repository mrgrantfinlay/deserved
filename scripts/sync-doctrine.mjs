...
const ROUTING_GENERATED = path.join(ROOT, "src/generated/routing-index.mjs");
const CAPABILITIES_GENERATED = path.join(ROOT, "src/generated/capabilities.mjs");
const ROUTING_SRC = path.join(OPPORTUNITIES_SRC, "_routing");
const CAPABILITIES_SRC = path.join(OPPORTUNITIES_SRC, "_capabilities/index.yaml");
...
function mergeOffer(doctrine, raw, jsonRelPath, opportunityFile, capabilityNameToId) {
  ...
  const capabilityIds = caps
    .map((c) => {
      if (typeof c === "string") return capabilityNameToId.get(c) ?? null;
      if (c.id) return c.id;
      if (c.name) return capabilityNameToId.get(c.name) ?? null;
      return null;
    })
    .filter(Boolean);
  ...
  return {
    ...
    problemSlug: doctrine.problem_slug ?? null,
    routingVertical: doctrine.routing?.vertical ?? null,
    cellStatus: doctrine.cell_status ?? "live",
    routingEligible: doctrine.routing_eligible !== false,
...
async function main() {
  ...
  try {
    const capRaw = parseYaml(await readFile(CAPABILITIES_SRC, "utf8"));
    capabilityIndex = capRaw.capabilities ?? {};
    capabilitiesVersion = capRaw.version ?? 0;
  } catch {
    console.warn("Skip capabilities: _capabilities/index.yaml not found");
  }
  const capabilityNameToId = new Map(
    Object.entries(capabilityIndex).map(([id, cap]) => [cap.name, id]),
  );

  const files = await readdir(OPPORTUNITIES_SRC);
  const mdFiles = files.filter(
    (f) => f.endsWith(".md") && f !== "README.md" && f !== "COMPILER.md",
  );
  ...
  for (const file of mdFiles) {
    ...
    const entry = mergeOffer(doctrine, raw, jsonRel, file, capabilityNameToId);
...
`;
  ...
  const routingFiles = ["problems.yaml", "needs.yaml", "markets.yaml", "classifier-v1.yaml"];
  const routing = {};
  for (const name of routingFiles) {
    const p = path.join(ROUTING_SRC, name);
    try {
      routing[name] = parseYaml(await readFile(p, "utf8"));
    } catch {
      console.warn(`Skip routing ${name}: not found`);
    }
  }

  const routingBody = `// AUTO-GENERATED by scripts/sync-doctrine.mjs — do not edit
export const PROBLEMS = ${JSON.stringify(routing["problems.yaml"]?.problems ?? {}, null, 2)};
export const NEEDS = ${JSON.stringify(
    {
      need_types: routing["needs.yaml"]?.need_types ?? {},
      problem_types: routing["needs.yaml"]?.problem_types ?? {},
    },
    null,
    2,
  )};
export const MARKETS = ${JSON.stringify(routing["markets.yaml"]?.markets ?? {}, null, 2)};
export const CLASSIFIER = ${JSON.stringify(routing["classifier-v1.yaml"] ?? {}, null, 2)};
export const ROUTING_SYNCED_AT = ${JSON.stringify(new Date().toISOString())};
`;
  await writeFile(ROUTING_GENERATED, routingBody, "utf8");

  const capabilitiesBody = `// AUTO-GENERATED by scripts/sync-doctrine.mjs — do not edit
export const CAPABILITIES = ${JSON.stringify(capabilityIndex, null, 2)};
export const CAPABILITIES_VERSION = ${JSON.stringify(capabilitiesVersion)};
export const CAPABILITIES_SYNCED_AT = ${JSON.stringify(new Date().toISOString())};
`;
  await writeFile(CAPABILITIES_GENERATED, capabilitiesBody, "utf8");

  console.log(
    `Synced ${offers.length} opportunity(ies) from 04-Opportunities/ + JSON-LD → catalog.mjs`,
  );
  console.log(`Synced routing index → routing-index.mjs`);
  console.log(`Synced ${Object.keys(capabilityIndex).length} capability(ies) → capabilities.mjs`);
}
